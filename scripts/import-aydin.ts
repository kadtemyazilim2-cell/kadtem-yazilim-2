...
async function main() {
    const filePath = path.join('C:\\Users\\Drone\\Desktop\\benzer iş grupları\\aydın\\aydın.html');
    ...
    function findSite(name: string) {
        ...
        // Specific Logic for Aydın
        if (cleanName.includes('Aydın') || cleanName.includes('Nazilli') || cleanName.includes('Yenipazar')) {
            return sites.find(s => s.name.includes('Aydın') && s.name.includes('Nazilli'));
        }

        // Fallback Logic from previous scripts
        if (cleanName.includes('Zile') || cleanName.includes('1.Kısım')) return sites.find(s => s.name.includes('Zile Ovası 1'));
    ...
    // We default to the Aydın site
    const defaultSite = sites.find(s => s.name.includes('Aydın') && s.name.includes('Nazilli'));
    if (!defaultSite) {
        console.error("Available Sites:", sites.map(s => s.name));
        throw new Error("Target Site 'Aydın Nazilli...' not found in DB!");
    }
    console.log(`Target Site: ${defaultSite.name}`);
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        const aracUst = aracKolon.find('.arac-ust').text().trim(); // Vehicle / Virman
        const aracAlt = aracKolon.find('.arac-alt').text().replace(/[()]/g, '').trim(); // Site Context
        ...
        if (!currentSite) currentSite = defaultSite;

        // Ensure Tank
        let tank = currentSite.fuelTanks[0];
        if (!tank) {
            tank = await prisma.fuelTank.create({
                data: { siteId: currentSite.id, name: 'Ana Tank', capacity: 20000, currentLevel: 0 }
            });
        ...
        // 2. CONSUMPTION
            ...
            if (!vehicle) {
                console.warn(`[WARN] Vehicle not found: ${aracUst} (Row ${i})`);
                counters.errors++;
                continue;
            }
        ...
        // 3. PURCHASE (Non-Virman, Positive)
        else if (liters > 0) {
            const exists = await prisma.fuelTransfer.findFirst({
