...
async function main() {
    const filePath = path.join('C:\\Users\\Drone\\Desktop\\benzer iş grupları\\zile1\\zile1.html');
    ...
    function findSite(name: string) {
        ...
        // Exact match preference for Zile 1
        if (cleanName.includes('Zile Ovası 1 Kısım') || cleanName.includes('Zile 1')) {
            // Prefer the main one, not Alt Yuklenici unless specified
            const mainSite = sites.find(s => s.name === "Tokat Zile Ovası 1 Kısım Arazi Toplulaştırma Ve Tarla İçi Geliştirme Hizmetleri Yapım İşi");
    ...
    // We default to the Zile 1 site if specific row site isn't found
    const defaultSite = sites.find(s => s.name === "Tokat Zile Ovası 1 Kısım Arazi Toplulaştırma Ve Tarla İçi Geliştirme Hizmetleri Yapım İşi");
    if (!defaultSite) {
        throw new Error("Target Site 'Tokat Zile Ovası 1 Kısım...' not found in DB!");
    }
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        if (!currentSite) currentSite = defaultSite; // Fallback to Zile 1
